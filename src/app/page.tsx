  ...
  const handleFetchData = async () => {
    ...
    try {
      ...
      const params = new URLSearchParams({
        figmaUrl: encodeURIComponent(figmaUrl),
        includeStyles: 'true',
    ...
    <div className="flex h-screen w-screen bg-white overflow-hidden">
      <aside className="w-72 bg-white border-r border-gray-200 p-4 flex flex-col space-y-4 overflow-y-auto">
          ...
          {nodes.length > 0 && nodes[0] && (
            <div className="flex-grow overflow-y-auto border rounded-md p-3 bg-white space-y-3">
                <h3 className="text-sm font-semibold mb-2">{nodes[0].name} (Top Level)</h3>
                {nodes[0].children?.slice(0, 15).map(child => (
                    <div key={child.id} className="text-xs text-gray-600 truncate pl-2 border-l-2 border-gray-200 hover:bg-gray-50 py-0.5">
                        {child.name} <span className='text-gray-400'>({child.type})</span>
                    </div>
                ))}
                {nodes[0].children && nodes[0].children.length > 15 && (
                    <div className="text-xs text-gray-400 pl-2">... and more</div>
                )}
                 {nodes[0].children?.length === 0 && (
                    <div className="text-xs text-gray-400 pl-2">No children nodes found.</div>
                )}
            </div>
          )}
      </aside>
      ...
      <main className="flex-grow flex flex-col overflow-hidden relative bg-gray-100"> 
          ...
          >
            ...
            >
              {status === 'success' && nodes.length > 0 && nodes.map(node => renderNode(node, scale))}
            </div>
