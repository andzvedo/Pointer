...
function processNodeForRendering(node: any): any {
  ...
  if (processedNode.children && Array.isArray(processedNode.children)) {
    // Adicionar referência ao nó pai para cada filho
    processedNode.children = processedNode.children
      .map((child: any) => {
        const processedChild = processNodeForRendering(child);
        if (processedChild) {
...
export async function GET(request: NextRequest) {
  ...
  try {
    const response = await fetch(figmaApiUrl, {
      ...
      next: { revalidate: 60 } // Cache por 60 segundos para evitar muitas requisições à API
    });
